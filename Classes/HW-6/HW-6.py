...
json_objs = RDD.map(lambda json_valid: safe_parse(json_valid)).filter(lambda h: "text" in h).map(lambda json_obj: (json_obj["user"]["id_str"], json_obj["text"])).map(lambda (x,y): (x, y.encode("utf-8"))).cache()
...
print_users_count(json_objs.map(lambda x:x[0]).distinct().count())
...
users_partition = pickle.load(open(path, 'rb'))
users_part = sc.broadcast(users_partition)
...
user_post = json_objs.map(lambda x:users_part.value.get(x[0], 7)).countByValue().items()
...
unique_tokens = json_objs.map(lambda json_obj: (json_obj[0], tok.tokenize(json_obj[1]))).flatMap(lambda t: splitter(t)).distinct()

ut1 = unique_tokens.map(lambda x: ((users_part.value.get(x[0],7), x[1]), 1)).cache()
...
print_count(popular_tokens), print_tokens(popular_tokens.top(20, lambda x:x[1]))
...
twg = sc.parallelize(users_partition.items()).rightOuterJoin(json_objs).map(lambda (uid,(gid,tweet)): (uid,(7,tweet)) if gid<0 or gid>6 else (uid,(gid,tweet))).cache()
#
